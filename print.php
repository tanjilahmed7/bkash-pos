...
if ($Data->id) {
    ?>
    <table width="270" border="0" cellpadding="0" cellspacing="0" class="booth_ticket" style="padding-left:0px;padding-top:5px;font-family: sans-serif;">
        ...
        <tr>
            <td align="left" valign="top" width="190" height="10" style="padding-left:25px;padding-top: 10px;">
                ...
                <div style="letter-spacing: 0px;padding-left: 3px;padding-top: 3px;font-size: 14px;">
                    <?php
                       $stmt = $auth_user->runQuery("SELECT ticket_category.name FROM `profession_category` LEFT JOIN ticket_category ON profession_category.profession=ticket_category.id WHERE reg_table_id = '$id'");
                       $stmt->execute();  
                       $profession_query = $stmt->fetchAll(PDO::FETCH_ASSOC);
                        echo $profession_query[0]['name'];
                    ?> 
                </div>
            </td>

            <td align="center" valign="middle" style="padding-right: 10px;padding-top: 5px;float: right;">
                <div style="font-size:17px;padding-left:2px"><strong style="margin-left: 0px;">GATE</strong></div>

                <div style="padding-top: 0;padding-left: 0px;">
                    <div class="gate_number" style="padding: 0px 0px 0px 0px; width:50px; height: 38px; font-size:30px; text-align: center; color: white; background-color: black;-webkit-print-color-adjust: exact; border: 1px solid;-webkit-border-radius: 8px;-moz-border-radius: 8px;border-radius:8px;">
                    <?php echo $gate; ?>
                    </div>
                    <div class="sl">
